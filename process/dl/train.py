...
from models import cnn2d_classic, gcn_classic
...
def get_train_test_data(nums_subjects, directory_dataset, ratio):
    raws_train = []
    raws_test = []
    if VERBOSE:
    ...
    for ii in range(1, nums_subjects+1):
        if ii <= nums_subjects*ratio:
            raws_train = load_one_subject(raws_train, ii, directory_dataset)
        else:
            raws_test = load_one_subject(raws_test, ii, directory_dataset)
...
def check_args(args):
    ...
    return nums_subjects, ratio, model_name, model, output, verbose, save_model, batch_size, epochs, directory_dataset
...
if __name__ == "__main__":
    ...
    nums_subjects, ratio, model_name, model, output, VERBOSE, save_model, batch_size, epochs, directory_dataset = check_args(args)
    raw_train, raw_test = get_train_test_data(nums_subjects, directory_dataset, ratio)
    ...
    model = model(input_shape, n_channels)
